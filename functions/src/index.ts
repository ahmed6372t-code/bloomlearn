  ...
  async (request) => {
    ...
    if (!request.auth) {
      throw new HttpsError(
        "unauthenticated",
        "You must be signed in to bake material."
      );
    }
    ...
    if (!text || typeof text !== "string" || text.trim().length < 50) {
      throw new HttpsError(
        "invalid-argument",
        "Please provide at least 50 characters of study material."
      );
    }
    ...
    try {
      ...
      try {
        ...
        throw new HttpsError("internal", "Failed to parse extraction results.");
      }

      if (!extraction.facts || extraction.facts.length === 0) {
        throw new HttpsError(
          "internal",
          "Extraction returned 0 facts. Please try more specific material."
        );
      }

      // Enforce referential integrity
      const validFactIds = new Set(
        extraction.facts.map((f: any) => f.id)
      );
      (extraction.concepts || []).forEach((concept: any) => {
        concept.fact_ids = (concept.fact_ids || []).filter((fid: string) =>
          validFactIds.has(fid)
        );
      });

      const validConceptIds = new Set(
        (extraction.concepts || []).map((c: any) => c.id)
      );
      (extraction.procedures || []).forEach((proc: any) => {
        proc.concept_ids = (proc.concept_ids || []).filter((cid: string) =>
          validConceptIds.has(cid)
        );
      });
